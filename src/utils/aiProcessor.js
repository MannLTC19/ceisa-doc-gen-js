...
//    Receives a page skeleton (page number + first 300 chars of each page).
//    Returns a JSON map: { background: [1,2], problems: [4,5], ... }
//
//  PASS 2 — Deep Analysis  (claude-opus-4-6, thorough)
//    Receives ONLY the relevant pages identified in Pass 1.
//    Produces the full structured project JSON.
//
//  Result: ~65% fewer Opus input tokens, ~20-30s vs 60-90s, better accuracy.
// ─────────────────────────────────────────────────────────────────────────────
...
const ANALYSIS_MAX_TOKENS = 16000; // Opus output cap — safe for proxy limits
...
OUTPUT PRIORITY ORDER — write fields in this exact order. Stop cleanly if token limit is near.
TIER 1 (CRITICAL — always complete):
  actors, useCases, kebutuhanFungsional, kebutuhanNonFungsional, risikoBisnis, asIsToBe
TIER 2 (IMPORTANT):
  nama, pengampu, unitPenanggungJawab, namaPIC, kontakPIC, targetPenyelesaian, targetOutcome, bia, detectedPeople
TIER 3 (OPTIONAL — only if tokens remain):
  latarBelakang, masalahIsu, outcomeKeluaran, businessValue, alurBisnisProses, brdProcessAnalysis, fsdLinks, mermaid

EXTRACTION RULES:
- actors: every human role, system, or external service that interacts with the system
  type: GUI=human via browser, Protocol=system-to-system, API=internal service
- useCases: every feature/activity from document; name as "Verb Noun" in Indonesian
...
- kebutuhanNonFungsional: min 6 items covering Security, Performance, Availability, Scalability, Compliance, Usability
- risikoBisnis: min 5 items; every integration point, manual process, and regulatory requirement = 1 risk
- asIsToBe: min 5 items; contrast current manual/legacy state vs target automated state
- bia: calibrate by process criticality — Critical→RTO:1h, High→RTO:4h, Medium→RTO:8h, Low→RTO:24h
- mermaid: STRICT LIMITS — max 12 nodes per diagram. Use double quotes only. No semicolons.
  processFlow: flowchart TD, top 8 main steps only
  useCaseDiagram: flowchart LR, actors + top 8 use cases only
  erd: erDiagram, top 5 core entities only, one field per line
  JSON-encode diagrams: newlines as \n, quotes as \"

MINIMUMS: actors≥5, useCases≥8, kebutuhanFungsional≥10, kebutuhanNonFungsional≥6, risikoBisnis≥5, asIsToBe≥5

CRITICAL OUTPUT RULES:
1. Return ONLY pure JSON. No markdown, no backticks. Start with { end with }
2. All strings must be properly JSON-escaped
3. Arrays must never be null — use []
4. Write TIER 1 fields before any prose fields
...
const analyzeDocument = async (apiKey, documentText, contextNote = "") => {
  ...
  const ATTEMPT_LIMITS = [120_000, 80_000, 50_000];
